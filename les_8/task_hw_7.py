...
if __name__ == "__main__":
    ...
    with open('.\\les_8\\files\\hw\\hw_pickle.pickle', 'rb') as ff:
        diction_pickle = pickle.load(ff)
    print(diction_pickle)

    # with open('.\\les_8\\files\\hw\\hw_csv.csv', 'r', encoding='utf-8') as ff:
    #     diction_csv = {}
    #     csv_read = csv.DictReader(ff)
    #     for line in csv_read:
    #         _, name, parrent_folder, _type, size = line.items()
    #         diction_csv.setdefault(parrent_folder[1], dict([name, parrent_folder, _type, size]))
    #         for path_, info in diction_csv.items():
    #             if path_ == parrent_folder[1]:
    #                 info.setdefault(path_, dict([name, parrent_folder, _type, size]))
    #     print(diction_csv)