...
def give_up(total_sum, input_sum):
    total_sum += input_sum
    return total_sum
...
def give_up_operation(total_sum, count_operations):
    while True:
        sum_for_give_up = int(input(f"Какую сумму желаете внести? \
Введите значение, кратное {FREQUENCY_RATE_SUM}: "))
        if check_input(sum_for_give_up):
            total_sum = give_up(total_sum, sum_for_give_up)
            total_sum = give_percent_for_operations(total_sum, count_operations)
...
Введите значение, кратное {FREQUENCY_RATE_SUM}: "))
        if check_input(sum_for_take_off):
            ...
            if check_take_off(sum_for_take_off, total_sum, percent_for_take):
                total_sum = take_off(total_sum, sum_for_take_off, percent_for_take)
                total_sum = give_percent_for_operations(total_sum, count_operations)
                print_balance(total_sum)
                return total_sum
            else:
                ...
                print_balance(total_sum)
                main_menu(total_sum, count_operations)
        else:
            print("Сумма должна ьыть кратна 50!")
            print_balance(total_sum)
...
Будет списано {RICH_LIMIT_PERCENT * 100:.2f} % от суммы остатка!")
        return total_sum * (1 - RICH_LIMIT_PERCENT)
    return total_sum
...
Вам начислено {PERCENT_FOR_OPERATIONS * 100:.2f} % от суммы остатка")
        return total_sum * (1 + PERCENT_FOR_OPERATIONS)
    return total_sum
...
\t4 - Выход")
        ...
        match input_menu:
            case 1:
                return give_up_operation(total_sum, count_operations)
            case 2:
